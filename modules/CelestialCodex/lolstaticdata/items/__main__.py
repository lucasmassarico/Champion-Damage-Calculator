...
from modules.CelestialCodex.lolstaticdata.items.pull_items_wiki import WikiItem, get_item_urls
# from .pull_items_dragon import DragonItem
from modules.CelestialCodex.lolstaticdata.items.pull_items_dragon import DragonItem
...
def main():
    ...
    for name, data in wikiItems.items():
        item = None
        print(name)
        item_with_html_tag = [x for x in cdragon if "id" in data and x["id"] == data["id"]]

        for i in item_with_html_tag:
